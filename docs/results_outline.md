...
Image-level category-prior diagnostic:

| Relabel prior | Class-aware AP | Class-aware AP50 | Class-aware AP75 |
|---|---:|---:|---:|
...
| torchvision ResNet18 ImageNet prior | 0.0663 | 0.0886 | 0.0776 |
...
| max three crops + nearest-GT category, original score | - | 0.3712 |
| max three crops + nearest-GT category, IoU score | - | 0.7515 |
...
- The max3 oracle split makes the remaining gap explicit: nearest-GT category with original scores reaches AP50 `0.3712`, and nearest-GT category with IoU scores reaches AP50 `0.7515`. This is the clearest current evidence that category prediction and score-IoU ranking dominate the next bottleneck.
- The category-prior coverage diagnostic shows that ResNet50's max3 top-5 setting covers any GT category in only `0.3913` of images, even though the largest-category top-5 is `0.2899`. The remaining oracle gap is therefore not only a score-calibration issue; the ImageNet teacher often misses the DET image categories entirely.
...
- ResNet50 top-5 multiply also improves the hard spatial slice: offcenter AP50 rises from the ResNet50 top-1 prior's `0.1012` to `0.1465`, while center AP50 rises only from `0.0910` to `0.1010`.
- A first calibration/heldout split utility (`scripts/split_coco_by_images.py`) confirms that top-k expansion is not just a full-eval artifact. On a 34-image calibration split, ResNet50 top-1/top3/top5/top10 AP50 is `0.1002/0.1033/0.1033/0.1037`; on the 35-image heldout split, it is `0.1161/0.1349/0.1634/0.1655`. The split is too small to tune K precisely, but it supports top-k expansion as a real category-transfer signal.
