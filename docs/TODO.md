...
- Stronger-teacher caution: using the staged 1+3 student as a train-split teacher is negative under the current `score>=0.25/top20` pseudo-label protocol. Its train-split prediction quality is lower than the original Nano5 teacher (`AP50 0.3137` vs `0.4474`, loc AP50 `0.7380` vs `0.7814`), and teacher-only training from those pseudo labels gives only class AP50 `0.0504`, below the original Nano5 teacher-only `0.0552`. Small 384px 3ep is also a weak train teacher despite using a larger RF-DETR model: raw train AP50 `0.2464` and filtered `s025/top20` AP50 `0.1728`, both far below Nano5. Resumed Small 5ep is better as a detector (`test AP50 0.1835`) and raw train teacher (`AP50 0.4041`, loc AP50 `0.7764`), but its filtered pseudo-label quality still trails Nano5 (`s025/top20` AP50 `0.3025` vs `0.3689`; `s015/top20` AP50 `0.3507` vs `0.4098`). Artifact: `results/rfdetr_train_teacher_quality_summary.csv`. Do not assume a better heldout detector is a better pseudo-label teacher; next pseudo-label work should improve teacher selection/filtering from a genuinely stronger checkpoint, not recycle staged13 or Small checkpoints by default.
- Pseudo-label filtering diagnostic added: `scripts/coco_annotations_to_predictions.py` converts selected pseudo-label COCO annotations back into detection predictions using `teacher_score`, so pseudo-label filters can be evaluated directly against train GT before spending MPS time on RF-DETR training. Artifact: `results/rfdetr_pseudo_filter_quality_summary.csv`. Result: stricter Nano5 filters are worse because recall collapses (`s025_top20` class AP50 `0.3689`; `s035_top10` `0.2752`; `s050_top10` `0.1753`), while wider filters improve selected pseudo quality (`s020_top20` class AP50 `0.3991`; `s015_top20` `0.4098`, loc AP50 `0.7565`). `s025_top10` also loses vs `s025_top20`, so top-k truncation is harmful here. Staged13 remains weaker than Nano5 (`s025_top20` class AP50 `0.2255`). Small-resume5 improves over Small3 but still trails Nano5 under all checked filters (`s015/top20` AP50 `0.3507`, `s020/top20` `0.3340`, `s025/top20` `0.3025`). Training check: despite stronger direct pseudo-label AP, wider Nano5 filters collapse in teacher-only 1ep training: `s015/top20` gives class AP50 `0.0120`, loc AP50 `0.1509`; `s020/top20` gives class AP50 `0.0119`, loc AP50 `0.1852`; both are far below `s025/top20` teacher-only (class AP50 `0.0552`, loc AP50 `0.5499`). Do not spend staged 1+3 runs on `s015/top20` or `s020/top20`; the current self-teacher sweet spot remains Nano5 `s025/top20` pretrain -> GT finetune until a stronger train-split teacher is found. The mismatch suggests low-score pseudo boxes can be good under static AP evaluation yet destabilize RF-DETR training.
...
- Long-train Small detector-side class head now becomes the strongest RF-DETR route. The resumed Small 384px 5ep regular checkpoint reaches independent-test class AP50 `0.2405`, AP75 `0.1795`, class-agnostic AP50 `0.6352`, offcenter AP50 `0.2238`, small AP50 `0.2457`, and large AP50 `0.3768`; extending to the resumed 8ep run improves class AP50 further to `0.2677`, AP75 `0.2053`, center AP50 `0.2527`, and small AP50 `0.2594`. This is substantially above the previous Nano GT-only / staged-pseudo 4ep range around AP50 `0.150-0.162`. Small-resume8 is also the first checked train-split teacher to beat Nano5 both raw (`AP50 0.4599` vs `0.4474`) and after filtering (`s025/top20 AP50 0.3832` vs `0.3689`; `s015/top20 AP50 0.4221` vs `0.4098`). Next active step: use Small-resume8 pseudo labels in a teacher-only pretrain -> GT finetune schedule; avoid raw GT+pseudo concatenation.
- Small-resume8 `s025/top20` teacher-only 1ep is positive but weak on independent test: class AP50 `0.0547`, AP75 `0.0504`, loc AP50 `0.3288`, offcenter AP50 `0.0437`. After 3 GT-finetune epochs from that pseudo-pretrained checkpoint, Nano reaches class AP50 `0.1855`, AP75 `0.1386`, loc AP50 `0.6300`, offcenter AP50 `0.1486`, small AP50 `0.1269`, and large AP50 `0.3164`. This is better than the earlier Nano5 staged seed41 result (`0.1502`) and equal-budget Nano GT-only (`0.1517`), but still far below direct Small-resume8 (`0.2677`). Treat stronger-teacher pseudo-pretrain as a useful weak-student warm-start, not a replacement for direct Small detector-side long training.
