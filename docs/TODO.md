...
- RF-DETR local package check: AIAA currently has `transformers` but not `rfdetr`, `roboflow`, `supervision`, or `timm`; no local RF-DETR repo/files were found under this project. Do not block the current engineering path on RF-DETR installation. The stable interface is now COCO JSON + prediction JSON + protocol stage-gate; RF-DETR should plug into that later by producing COCO-format predictions.
...
- RF-DETR controlled local install update:
  - Minimal AIAA install now reaches `import rfdetr` and `import supervision` after installing `rfdetr --no-deps`, replacing the wrong `deprecate` package with `pyDeprecate==0.7.0`, and adding `peft`, `accelerate`, `supervision`, `protobuf>=5.28,<6`, `transformers==5.8.1`, `huggingface-hub>=1.5`, and `regex>=2025.10.22`.
  ...
  - `results/rfdetr_offcenter_seed41_handoff_check.json` now performs real import smoke checks and confirms `rfdetr`, `torch`, `torchvision`, `transformers`, and `supervision` are importable; `timm` remains absent. The dataset check now scans all split image files and validates annotation image ids plus bbox area/bounds.
  ...
  - Current blocker moved from package import to pretrained-weight/training validation. `RFDETRNano()` starts downloading `rf-detr-nano.pth` (`349MB`); do not claim RF-DETR training is runnable until this weight download and a tiny training/inference smoke complete.
