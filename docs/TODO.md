...
- RF-DETR controlled local install update:
  ...
  - Prediction overlay for the fixed failure set added: `scripts/visualize_coco_sample_predictions.py` writes `results/rfdetr_stratified_seed41_candidate_failure_prediction_overlays/contact_sheet.jpg` plus `manifest.csv`. In the current 30-sample contact sheet, the nearest grouped RF-DETR prediction has mean IoU `0.784` and reaches IoU >= `0.5` on `24/30` samples, but the GT category appears in the displayed candidate group for only `2/30` samples. This is the clearest qualitative artifact so far that many fixed failure samples are localized but semantically missing from the emitted candidate set.
  - Fixed-sample prediction-chain summary added: `scripts/summarize_coco_sample_prediction_chain.py` writes `results/rfdetr_stratified_seed41_candidate_failure_prediction_chain.csv` and `_summary.csv`. Across all 52 fixed target annotations, mean nearest IoU stays high through `base/full_lr_2ep/extra_low_lr` (`0.811/0.810/0.822`), but correct-category candidate presence falls `4/52 -> 3/52 -> 0/52`. This makes the extra-low-LR interpretation stricter: it can improve deployed class AP while regressing the concrete fixed failure categories' candidate emission.
