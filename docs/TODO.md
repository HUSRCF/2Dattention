...
- Current stop point: the offcenter issue is now a serious assignment/ranking-direction problem, not an implementation bug. Simple spatial query seeds, hard grid reference centers, soft grid reference gates, and minimal DAB-style learned reference boxes all expose AP/geometry trade-offs without solving object-specific offcenter binding. Do not add another small reference heuristic. The next credible step requires either (1) a proper multi-layer DAB/Deformable-DETR-style decoder with reference updates and matching diagnostics, (2) explicit query-target binding supervision, or (3) moving to a real detector baseline where query assignment/ranking machinery is not this underpowered.
