...
- Teacher-only pretrain -> GT finetune is now the first positive detector-side distillation schedule. Using Nano 5ep self-teacher pseudo labels for 1 epoch, then finetuning on GT for 1 epoch, gives class AP50 `0.0816`, above GT 1ep (`0.0495`), teacher-only (`0.0552`), and raw GT+pseudo (`0.0484`), with class-agnostic AP50 `0.5749`. Extending the GT finetune to 2 epochs strengthens the result: class AP50 `0.1088`, AP75 `0.0820`, class-agnostic AP50 `0.6273`, off-center AP50 `0.1163`, and large-object AP50 `0.2240`. Caveat: RF-DETR's `best_total` checkpoint selected a stale/high EMA (`0.7167`) in this flow, so reported staged-finetune results use `checkpoint_best_regular.pth` and independent `predict_rfdetr_coco.py` export. Next: stronger train-split teacher predictions or a more formal longer protocol; do not use raw GT+pseudo concatenation.
