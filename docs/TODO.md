...
- Continuing the larger split from the 2ep best regular checkpoint for 4 more epochs with a fresh optimizer at `lr=1e-4` gives a strong positive direction signal: test class AP50 rises to `0.2196`, AP75 to `0.1841`, AP to `0.1746`, class-agnostic AP50 to `0.6111`, offcenter AP50 to `0.2073`, small AP50 to `0.1890`, medium AP50 to `0.2248`, and large AP50 to `0.3263`. This is still below the old 330-image/20ep peak because the new protocol is harder and less trained, but the delta from the 2ep gate is large. Current decision: the larger split should be treated as a formal long-train route, not as a failed smoke. If continuing, extend this same protocol; do not return to alpha/LR micro-sweeps or persistent proposal state.
