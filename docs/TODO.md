...
- Hard-category oversampling hook added: `scripts/build_coco_hard_category_oversample.py` builds an RF-DETR-compatible train split by duplicating images containing candidate-missing categories while preserving valid/test. The original absolute-symlink smoke dataset `/private/tmp/rfdetr_stratified_seed41_train1000_hardcat_oversample60_abs` passes RF-DETR `--check-only`; train images/annotations become `1000/4409 -> 1087/5221`. Later audit found the early builder could assign duplicate train image ids that collide numerically with valid/test ids, even without file-name leakage. The builder now reserves ids across train/valid/test, and future hardcat datasets must be regenerated before use. Artifact: `results/rfdetr_stratified_seed41_hardcat_oversample60_abs_summary.csv`.
- Fixed hard-category oversampling dataset regenerated at `/private/tmp/rfdetr_stratified_seed41_train1000_hardcat_oversample60_fixed`. It preserves the same train expansion (`1000/4409 -> 1087/5221`) but passes both numeric image-id and file-name split-disjoint checks: `image_id_overlap_counts={train_valid: 0, train_test: 0, valid_test: 0}` and `file_name_overlap_counts={train_valid: 0, train_test: 0, valid_test: 0}`. RF-DETR `--check-only --num-classes 200` also accepts it. Artifacts: `results/rfdetr_stratified_seed41_hardcat_oversample60_fixed_summary.csv`, `results/rfdetr_stratified_seed41_hardcat_oversample60_fixed_handoff.json`.
...
- Integrated RF-DETR class-head protocol: `results/rfdetr_integrated_class_head_summary.csv` evaluates full prepared split RF-DETR detector-side finetunes with exported `model_num_classes=200`; the export script now also force-checks and prints actual detection-head out features, so future rows should use that actual-head print rather than argument echo alone. Nano 384px improves with longer training: class-aware AP50 rises from `0.0495` at 1 epoch to `0.1224` at 3 epochs and `0.1437` at 5 epochs; class-agnostic AP50 remains much higher (`0.5584 -> 0.6074 -> 0.6214`), supporting category/class-head learning as the active bottleneck. The 5-epoch Nano result still trails the current max3 crop + ResNet50 top-5 prior AP50 `0.1697`, but it currently points to the integrated detector-side route. Small 384px improves from AP50 `0.0513` at 1 epoch to `0.1178` at 3 epochs and `0.1835` after resumed 5-epoch training, with class-agnostic AP50 `0.6285`. This makes resumed Small the strongest validation-mirrored integrated detector-side checkpoint so far, but not automatically a stronger pseudo-label teacher. Do not return to post-hoc crop priors; next step is stronger teacher selection/filtering or a formal long-train detector-side protocol.
...
- Semantic hard-negative training substrate added after the fixed-candidate failure analysis. `DetectionCriterion` now supports an optional semantic hard-negative margin loss, and `scripts/convert_semantic_repair_config_to_loss_map.py` converts the COCO-category repair config into an explicit class-index loss map with required `--category-id-offset`. Dry-run artifact: `results/rfdetr_stratified_seed41_semantic_repair_loss_map.json` generated with `category_id_offset=1`. `scripts/train_det_real.py` can consume this map through `--semantic-hard-negative-loss-map`; when category names are present it remaps them onto the active runtime `label_to_id`, avoiding silent misuse of RF-DETR category ids as local compact logits indices. The runner now prints train image/box coverage for mapped hard-negative targets and supports `--train-semantic-hard-negative-oversample-factor` plus `--semantic-hard-negative-sample-mode {prioritize,only}`. A generic `max_samples=160` smoke only covered `1` train image / `1` train box; with oversample factor `20`, the semantic loss path activates (`loss_semantic_hard_negative≈1.92` at step 5). Prioritized sampling raises coverage to `128` train images / `166` train boxes on the same max-sample budget, making it the right targeted smoke mode. This remains only a low-risk training interface for future class-head/semantic repair tests; it is not yet RF-DETR evidence that hard-negative loss improves candidate coverage.
...
- Active-route selector added: `scripts/select_rfdetr_stage_gate_rows.py` ranks rows across protocol summaries. Current artifact `results/rfdetr_active_route_top_class_ap50.csv` ranks `rfdetr_indtest_seed43_detector_side_summary.csv` plus `rfdetr_random_seed41_train1000_protocol_summary.csv` by class AP50. Top rows are independent-test Small long-resume seed41 `0.3625/0.3546`, followed by random train1000 14best `0.3242`; use this table to plan the next formal RF-DETR run while keeping protocol differences explicit.
- Slice-specific active-route tables added: `results/rfdetr_active_route_top_offcenter_ap50.csv` and `results/rfdetr_active_route_top_small_ap50.csv`. The seed41 20ep row remains best on both (`offcenter=0.3241`, `small=0.3334`), but random train1000 14best is competitive on the same hard slices (`offcenter=0.3064`, `small=0.2980`). This supports continuing the larger train1000 protocol for the next formal RF-DETR run rather than returning to small-protocol sweeps.
- Multi-metric active-route ranking added: `scripts/rank_rfdetr_stage_gate_rows.py` writes `results/rfdetr_active_route_multi_metric_rank.csv` using mean rank over `class_ap50`, `offcenter_ap50`, `small_ap50`, and `loc_ap50`. It keeps the historical seed41 long-resume/fresh-LR rows at the top, but the random train1000 10best and 14best rows rank `4.25` and `6.25` respectively, supporting train1000 as the next practical formal route to continue. Treat this as a planning guardrail only; it mixes protocol summaries and is not a direct benchmark equivalence claim.
...
- Stratified-only active-route ranking added: `results/rfdetr_stratified_active_route_multi_metric_rank.csv` ranks `results/rfdetr_stratified_mainline_summary.csv` by the same class/offcenter/small/loc AP50 mean-rank guardrail. The current best stratified continuation is `seed41_fulllr5e5_then3e5_2ep` (`mean_rank=1.25`), followed by `seed41_fulllr5e5_2ep` (`1.75`). Use this table, not the mixed random/indtest route table, when selecting the next serious stratified RF-DETR continuation.
- Next formal RF-DETR run plan added: `docs/rfdetr_next_formal_run.md` selects the stratified `seed41_fulllr5e5_then3e5_2ep` regular checkpoint for a fresh-optimizer `lr=2e-5` 2-epoch continuation. A one-image MPS export probe passed with `actual_class_head_out_features=201`; summary artifact: `results/rfdetr_stratified_seed41_extra_low_lr_head_probe_summary.json`. Post-run evaluation is now bundled by `scripts/evaluate_rfdetr_run_artifacts.py`, and `scripts/update_rfdetr_mainline_summary.py` updates the mainline summary plus stratified route rank from that bundle. This is a prepared command plan only; the long run has not been started.
...
- RF-DETR controlled local install update:
  ...
  - Formal extra-low-LR continuation from the `lr=3e-5` checkpoint at lr `2e-5` for 2 more epochs is now complete under the standard train1000/valid200/test200 stratified protocol. Independent test export from `checkpoint_best_regular.pth` gives class AP/AP50/AP75 `0.2100/0.2700/0.2288` and class-agnostic loc AP/AP50/AP75 `0.4280/0.5837/0.4561`. It becomes the best stratified active-route row by multi-metric mean rank (`1.25`) and improves `class_ap50`, `small_ap50`, and `loc_ap50`, but it does not repair candidate generation: candidate hit rate is `0.3828`, below the previous `lr=3e-5` checkpoint (`0.3981`) and the earlier `lr=5e-5` checkpoint (`0.4060`). High-support weighted hit is `0.3855` with `8` zero-hit high-support categories. Interpretation: the extra training is a useful detector-side AP/loc continuation, but not a category-candidate breakthrough; do not keep extending seed41 solely to chase candidate coverage.
  - Second-seed full-model continuation confirms the detector-side class-AP gain but not candidate repair. Starting from the repaired-head seed43 2ep checkpoint and training all parameters for 2 more epochs at lr `5e-5` gives class AP/AP50/AP75 `0.2165/0.2754/0.2391`, improving sharply over seed43 forced 2ep (`0.1474/0.1888/0.1608`) and becoming the highest class AP50 row in the stratified summary. However loc AP/AP50/AP75 is only `0.3783/0.5235/0.4055`, offcenter/small AP50 are `0.1866/0.1563`, and candidate hit rate drops to `0.3562` with high-support weighted hit `0.3618` and `12` zero-hit high-support categories. Interpretation: full detector-side adaptation is a reproducible class-score/semantic learning route, but it does not reliably improve spatial robustness or candidate coverage. Future work should not claim category-candidate repair from full continuation alone.
  ...
  - Candidate failure category report added: `scripts/summarize_candidate_failure_categories.py` joins the seed41 transition table, split counts, and base high-IoU confusion flows into `results/rfdetr_stratified_seed41_candidate_failure_categories.csv`. The most actionable persistent/regressed zero-hit rows include `n03676483` (`20` train boxes, `11` test boxes, latest nearest IoU `0.640`, top wrong flow `n07880968`), `n07695742` (`20` train, `9` test, nearest IoU `0.612`, top wrong flow `n01726692`), and `n04468005` (`8` train, `3` test, nearest IoU `0.412`, top wrong flow `n04530566`). This is the current target list for qualitative category-candidate debugging; do not treat the zero-hit issue as an abstract aggregate.
  - Test-split sample list added for those failure categories: `scripts/select_coco_category_samples.py` outputs `results/rfdetr_stratified_seed41_candidate_failure_test_samples.csv` with `52` GT annotations across persistent/regressed zero-hit categories. This is the fixed image/category target set for future prediction overlays or manual inspection.
  - GT-only visualization for the fixed failure set added: `scripts/visualize_coco_category_samples.py` writes `results/rfdetr_stratified_seed41_candidate_failure_gt_overlays/contact_sheet.jpg` plus a manifest. The contact sheet confirms the failure set includes large clear objects, small target boxes, and repeated multi-instance scenes; use the individual overlay files/manifest for report curation rather than relying on the dense contact sheet alone.
  - Prediction overlay for the fixed failure set added: `scripts/visualize_coco_sample_predictions.py` writes `results/rfdetr_stratified_seed41_candidate_failure_prediction_overlays/contact_sheet.jpg` plus `manifest.csv` and `run_manifest.json`. In the current 30-sample contact sheet, the nearest grouped RF-DETR prediction has mean IoU `0.784` and reaches IoU >= `0.5` on `24/30` samples, but the GT category appears anywhere in the grouped candidate set for only `2/30` samples and never within the displayed top-5. This is a top-k nearest-box diagnostic, not a claim that the high-score deployed prediction is correct; it shows local box support often exists while emitted semantic candidates miss the target.
  - Fixed-sample prediction-chain summary added: `scripts/summarize_coco_sample_prediction_chain.py` writes `results/rfdetr_stratified_seed41_candidate_failure_prediction_chain.csv`, `_summary.csv`, and `_summary_run_manifest.json`. Across all 52 fixed target annotations, mean nearest IoU stays high through `base/full_lr_2ep/extra_low_lr` (`0.811/0.810/0.822`), but correct-category candidate presence falls `4/52 -> 3/52 -> 0/52`, with displayed-top-5 presence `0` throughout. Mean nearest-group rank drifts `10.2 -> 12.8 -> 13.8`, so the diagnostic is about available bbox support and candidate emission, not deployed top-score ranking. This makes the extra-low-LR interpretation stricter: it can improve deployed class AP while regressing the concrete fixed failure categories' candidate emission.
  - Fixed-sample candidate-flow aggregation added: `scripts/summarize_coco_sample_candidate_flows.py` writes `results/rfdetr_stratified_seed41_candidate_failure_candidate_flows.csv` and `_candidate_flow_by_category.csv`. Stable wrong-flow examples include base `n03676483 -> n00007846` (`10` samples, mean nearest IoU `0.888`), base `n07695742 -> n00007846` (`6`, IoU `0.969`), and continuation-stage `n07695742 -> n01726692` / `n04468005 -> n04530566`. Future class-head or teacher repair should target these concrete semantic-confusion flows, not only aggregate AP.
  - Candidate repair target table added: `scripts/build_rfdetr_candidate_repair_targets.py` writes `results/rfdetr_stratified_seed41_candidate_repair_targets.csv`. It merges split support, aggregate failure stats, high-IoU wrong-flow evidence, and checkpoint-specific dominant wrong candidates into one row per target category. This should be the fixed entrypoint for any next class-head/teacher repair experiment.
  - Hard confusion pair table added: `scripts/build_rfdetr_hard_confusion_pairs.py` writes `results/rfdetr_stratified_seed41_candidate_failure_hard_pairs.csv` and `_hard_pair_summary.csv`. With `min_iou=0.5` and full-group GT absence, it extracts `132` high-IoU hard instances and `73` aggregate GT-to-wrong-candidate pairs. This is the sample-level hard-negative substrate for future class-head contrastive loss, semantic teacher filtering, or targeted repair prompts.
  - Semantic repair config added: `scripts/build_rfdetr_semantic_repair_config.py` writes `results/rfdetr_stratified_seed41_semantic_repair_config.json`. It maps category names to COCO ids, merges duplicate `(GT, negative)` pairs across checkpoint labels, caps pair weights, and keeps example ids. With `min_samples=2`, it yields `6` target categories; the strongest targets are `n03255030`, `n03676483`, and `n07695742`. This JSON is the first training-consumable handoff for a future hard-negative class-head loss or teacher-side semantic repair.
  - Local semantic hard-negative loss substrate is now runnable but not yet positive. `scripts/convert_semantic_repair_config_to_loss_map.py` produces `results/rfdetr_stratified_seed41_semantic_repair_loss_map.json`, and `scripts/train_det_real.py` remaps those name-bearing entries onto the active compact label space before training. The targeted sampler `--semantic-hard-negative-sample-mode prioritize` fixes the original coverage issue (`1` image / `1` box under generic max-sampling) by raising target coverage to about `128` train images and `166-169` boxes under the same `max_samples=160` budget. However, the 50-step / 2-seed targeted smoke is not positive: `weight=0.0` reaches final/best IoU `0.312/0.326`, AP50 `0.198`, class AP50 `0.133`, score-IoU corr `0.530`, while `weight=0.1` reaches final/best IoU `0.314/0.331`, AP50 `0.172`, class AP50 `0.109`, score-IoU corr `0.158`. Conclusion: the hard-negative substrate is an engineering handoff, not a current metric-improving route. Do not scale the always-on local margin loss; if revisited, use lower weight, late start, or an RF-DETR-integrated class-head objective.
  - Semantic hard-negative loader guardrails added: duplicate positive entries are merged, duplicate negative pairs keep the larger weight, mapping/drop stats are printed, and nonzero semantic loss weight now fails early if the active label map has zero mapped repair targets. This avoids silent no-op mini-det repair runs.
  - Semantic repair dataset-coverage preflight added: `scripts/summarize_semantic_repair_dataset_coverage.py` writes `results/rfdetr_stratified_seed41_semantic_repair_dataset_coverage.csv`. All configured target/negative names are present in the RF-DETR category list, but top repair positives are low-support and validation-poor: `n03255030` has train/test boxes `21/14` and no valid boxes, `n03676483` has `20/11` and no valid boxes, and `n07695742` has only `1` train image with `20` boxes and no valid boxes. This makes always-on semantic margin loss an even weaker active route; use this file as a preflight if a future RF-DETR-integrated class-head repair objective is attempted.
  ...
  - Forced-head second-seed 2ep sanity completed on `rfdetr_stratified_seed43_train1000_val200_test200_min3`. The export script now force-checks the detection head and prints `actual_class_head_out_features`, so older `model_num_classes=200` messages should be treated as argument echoes rather than proof of actual head shape. Independent test metrics from `checkpoint_best_regular.pth` are class AP/AP50/AP75 `0.1474/0.1888/0.1608` and class-agnostic loc AP/AP50/AP75 `0.3635/0.5068/0.3868`; slice AP50 is `offcenter=0.1326`, `center=0.2097`, `small=0.0900`, `medium=0.2083`, `large=0.3923`. Candidate hit rate is `37.8%`, candidate-oracle AP/AP50/AP75 is `0.3729/0.4849/0.4079`, top-100 coverage is `loc=0.8079`, global class-aware `0.6382`, per-category class-aware `0.7000`, and score-IoU Spearman is weak (`loc=0.0506`, class-aware `0.1321`). Treat this as a repaired-head baseline sanity row only; it is not directly comparable to seed41's later full low-LR continuation, and it does not change the current long-train RF-DETR route.
  - Export-path actual-head smoke confirmed the updated predictor behavior on the real seed43 forced checkpoint: one-image export prints `model_num_classes=200`, `actual_class_head_out_features=201`, and `forced_class_head_out_features: 201 -> 201`.
  ...
  - Standard RF-DETR run artifact evaluator added: `scripts/evaluate_rfdetr_run_artifacts.py` writes class-aware COCOeval, class-agnostic loc COCOeval, slice AP50, score-IoU correlation, candidate coverage, per-category coverage, and candidate-oracle files from one prediction JSON. `scripts/update_rfdetr_mainline_summary.py` then updates `results/rfdetr_stratified_mainline_summary.csv` and regenerates `results/rfdetr_stratified_active_route_multi_metric_rank.csv`. Use this bundle path for future formal RF-DETR rows instead of hand-running scattered evaluators.
  - Artifacts:
