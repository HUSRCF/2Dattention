...
- Next formal RF-DETR run plan added: `docs/rfdetr_next_formal_run.md` selects the stratified `seed41_fulllr5e5_then3e5_2ep` regular checkpoint for a fresh-optimizer `lr=2e-5` 2-epoch continuation. A one-image MPS export probe passed with `actual_class_head_out_features=201`; summary artifact: `results/rfdetr_stratified_seed41_extra_low_lr_head_probe_summary.json`. This is a prepared command plan only; the long run has not been started.
