...
- RF-DETR controlled local install update:
  ...
  - Mainline summary helper added: `scripts/summarize_rfdetr_mainline.py` merges class-aware COCOeval, class-agnostic loc COCOeval, slice AP50, candidate-oracle diagnostics, top-k coverage, and score-IoU correlation into one stage-gate CSV. Current summary: `results/rfdetr_stratified_mainline_summary.csv`. This should be the quick comparison table for stratified RF-DETR regular-checkpoint runs before launching new long continuations.
  - Artifacts:
