...
- Current stop point: the offcenter issue is now a serious assignment/ranking-direction problem, not an implementation bug. Simple spatial query seeds, hard grid reference centers, soft grid reference gates, minimal DAB-style learned reference boxes, and naive reference-aware matching all expose AP/geometry trade-offs without solving object-specific offcenter binding. Do not add another small reference/matcher heuristic. The next credible step requires either (1) a proper multi-layer DAB/Deformable-DETR-style decoder with reference updates and mature matching diagnostics, or (2) moving to a real detector baseline where query assignment/ranking machinery is not this underpowered.
