...
- Integrated RF-DETR class-head smoke: `results/rfdetr_integrated_class_head_summary.csv` evaluates full prepared split RF-DETR Nano finetunes at 128px and 384px. The exported checkpoints report `model_num_classes=200`, so this is a real detector-side 200-class path. The 384px run improves from class-aware AP50 `0.0495` at 1 epoch to `0.1224` at 3 epochs, while class-agnostic AP50 stays much higher (`0.5584 -> 0.6074`). Interpretation: integrated detector training is the right direction and longer training helps, but 3 epochs still trails the current max3 crop + ResNet50 top-5 prior AP50 `0.1697`. Do not return to post-hoc crop priors; next step is a stronger teacher / distillation / longer detector-side class-head protocol.
