...
- Larger random RF-DETR continuation is strongly positive so far. Continuing the 2ep checkpoint for 4 more epochs with a fresh optimizer at `lr=1e-4` reaches class AP50 `0.2196`, and a second 4-epoch continuation reaches class AP50 `0.2912`, AP75 `0.2395`, AP `0.2190`, class-agnostic AP50 `0.6265`, offcenter AP50 `0.2603`, small AP50 `0.2458`, medium AP50 `0.2865`, and large AP50 `0.3646`. Artifact: `results/rfdetr_random_seed41_train1000_protocol_summary.csv`. This confirms the 1000-image protocol is the active RF-DETR route; class head training is still improving materially, so future progress should be planned as low-frequency longer detector-side runs rather than more old-split schedule sweeps.
...
- Attention manifest summarizer added: `scripts/summarize_rfdetr_attention_manifest.py` flattens image-level and query-level attention diagnostics into CSV. Current artifacts: `results/rfdetr_attention_query_overlays_train1000_6best/image_attention_summary.csv` and `results/rfdetr_attention_query_overlays_train1000_6best/query_attention_summary.csv`. In the 6-image off-center smoke, top-query GT attention mass has mean `0.605`, median `0.818`, and `2/14` zero-GT-mass query failures; `9/14` top queries have nearest-GT IoU >= `0.5`, with mean IoU `0.581` and median IoU `0.773`.
