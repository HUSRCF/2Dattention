...
- A valid-split post-hoc calibrator was applied to the 10best RF-DETR checkpoint without test-set alpha tuning. It improves score-IoU alignment (`class-aware Spearman 0.027 -> 0.291`, Pearson `0.361 -> 0.402`; loc Spearman `0.018 -> 0.120`) but barely changes class-aware COCOeval (`AP50 0.3155 -> 0.3158`, AP75 `0.2517 -> 0.2521`). Offcenter AP50 improves only slightly (`0.3081 -> 0.3120`) and small AP50 drops slightly (`0.2792 -> 0.2767`). Interpretation: a lightweight frozen calibrator can learn ranking signal, but it is not enough to solve the 10best full-set AP gap; do not continue same-split alpha/temperature sweeps.
