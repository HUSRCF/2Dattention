...
- Semantic hard-negative training substrate added after the fixed-candidate failure analysis. `DetectionCriterion` now supports an optional logits-indexed semantic hard-negative margin loss, and `scripts/convert_semantic_repair_config_to_loss_map.py` converts the COCO-category repair config into an explicit class-index loss map with required `--category-id-offset`. Dry-run artifact: `results/rfdetr_stratified_seed41_semantic_repair_loss_map.json` generated with `category_id_offset=1`. This is only a low-risk training interface for future class-head/semantic repair smoke tests; it is not yet RF-DETR evidence that hard-negative loss improves candidate coverage.
