...
- Slice-stratified ranking diagnostics added: real DET CSV rows now include `center_*` and `offcenter_*` metrics for matched assignment class accuracy, TP50 class accuracy, score-IoU correlation, objectness AUC, top-k FP rate, and duplicate-per-GT. Smoke artifact: `results/det_real_slice_diagnostics_smoke.csv`. Use these fields in the next off-center run before introducing another model variant.
